...
def hrnet_w32(pretrained=True, pretrained_ckpt=None):
    ...
    cfg = get_cfg_defaults(str(pretrained_ckpt), width=32)
    return get_pose_net(cfg, is_train=True)


def hrnet_w48(pretrained=True, pretrained_ckpt=None):
    if pretrained_ckpt is None:
        pretrained_ckpt = resolve_data_path('pretrained_models', 'pose_coco', 'pose_hrnet_w48_256x192.pth')
    else:
        pretrained_ckpt = resolve_asset_path(pretrained_ckpt)
    cfg = get_cfg_defaults(str(pretrained_ckpt), width=48)
    return get_pose_net(cfg, is_train=True)
